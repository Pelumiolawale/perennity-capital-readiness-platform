...
// Every VITE_* variable is inlined into the public client bundle. An Airtable
// PAT shipped that way in 2026 (anyone viewing source had read/write on the
// whole base). Refuse to build if a secret-looking VITE_ variable is present
// in the environment or in any .env file Vite would load. Secrets belong in
// unprefixed variables read by the /api serverless functions.
const SAFE_VITE_VARS = new Set(['VITE_ENGINE_COMMIT_SHA'])
const SECRET_NAME_RE = /(PAT|SECRET|TOKEN|KEY|PASSWORD)/i

export function findExposedSecrets(env) {
  return Object.keys(env).filter(
    (k) =>
      k.startsWith('VITE_') &&
      !SAFE_VITE_VARS.has(k) &&
      SECRET_NAME_RE.test(k) &&
      env[k],
  )
}

function assertNoExposedSecrets(mode) {
  const env = { ...loadEnv(mode, __dirname, 'VITE_'), ...process.env }
  const exposed = findExposedSecrets(env)
  if (exposed.length > 0) {
    throw new Error(
      `vite.config: refusing to build — ${exposed.join(', ')} would be inlined ` +
        'into the public bundle. Rename to an unprefixed variable (e.g. ' +
        'AIRTABLE_PAT) and read it from an /api serverless function.',
    )
  }
}
